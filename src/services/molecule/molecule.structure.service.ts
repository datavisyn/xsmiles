...
class MoleculeStructureService {
  ...
  getVerticesFromMolecule(molecule: Molecule): Vertex[] {
    if (isEmptyNullUndefined(molecule.smilesElements)) throw new Error('indexedSmilesElements is null or empty');
  ...
  public getRingContext = (indexedSmilesElements: SmilesElement[], start: number, visited: number[]) => {
    if (start + 1 === indexedSmilesElements.length) return { visited, lastIndex: start };
  ...
  public preprocessSmilesElementsAndMethod = (rawMolecule: RawMolecule): Molecule => {
    ...
    if (smilesSequence != null) {
      ...
      if (method != null) {
        ...
        if (method!.attributes != null) resultSmilesMethod.attributes = method!.attributes;
      }
    } else {
  ...
  private setRingsInSmilesElements(smilesElements: SmilesElement[], gVertices: any[], findGVertex: (gVertices: any[], smilesElement: SmilesElement) => any) {
    smilesElements.forEach((smilesElement) => {
      const gVertex = findGVertex(gVertices, smilesElement);
      smilesElement.rings = gVertex == null ? [] : gVertex.value ? gVertex.value.rings : gVertex.rings;
    });
  }
  ...
  public smilesElementIsAtom = (smilesElement: SmilesElement) => {
    ...
    if (smilesElement.chars === 'h' || smilesElement.chars === 'H') return false;
    return true;
  };
